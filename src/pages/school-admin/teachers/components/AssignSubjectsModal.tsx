...
import { assignSubjectsSchema } from '@/schemas/subject.schema';
import type { AssignSubjectsFormValues } from '@/schemas/subject.schema';
...
export default function AssignSubjectsModal({ isOpen, onClose, teacher }: AssignSubjectsModalProps) {
  ...
  const {
    ...
    reset,
    register,
  } = useForm<AssignSubjectsFormValues>({
  ...
  // Sync Form State with Teacher Data
  useEffect(() => {
    if (isOpen && teacher) {
      // Safely filter out undefined IDs just in case
  ...
  // Enterprise Feature: Search/Filter Logic
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all"
                value={searchTerm}
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <>
                            ...
                            filteredSubjects.map((subject) => {
                              ...
                              return (
                                <label
                                  key={subject.id}
                                  className={`
                                    relative flex items-center space-x-3 rounded-xl border p-4 cursor-pointer transition-all duration-200
                                    ${isSelected 
                                ...
                                >
                                  <input
                                    ...
                                    checked={isSelected}
                                    onChange={(e) => {
                                      // Manually manage the array state. Bulletproof!
                                      if (e.target.checked) {
                                  ...
                                  {isSelected && (
                                    <CheckCircle2 className="absolute top-2 right-2 h-4 w-4 text-primary-600" />
                                  )}
                                </label>
                              );
                            })
                        ...
                        {/* Selected Counter embedded dynamically */}
